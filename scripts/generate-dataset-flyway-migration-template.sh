...
TARGET_PROJECT_DIR="$PROJECT_BASE_DIR/subprojects/dataset-flyway-migration-template"
...
sync_source_with_repository() {
  ...
  then
    ...
    git clone -b master \
        https://github.com/laplacian-arch/dataset.flyway-migration-template.git \
        $TARGET_PROJECT_DIR
  else
...
project:
  ...
  name: dataset.flyway-migration-template
  type: template
  ...
  source_repository:
    url: https://github.com/laplacian-arch/dataset.flyway-migration-template.git
    branch: master
  module_repositories:
    local:
      ../../../mvn-repo
...
show_usage () {
cat << 'END'
Usage: ./scripts/generate-dataset-flyway-migration-template.sh [OPTION]...
  -h, --help
