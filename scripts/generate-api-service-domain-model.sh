...
TARGET_PROJECT_DIR="$PROJECT_BASE_DIR/subprojects/api-service-domain-model"
...
sync_source_with_repository() {
  ...
  then
    ...
    git clone -b master \
        https://github.com/laplacian-arch/api-service.domain-model.git \
        $TARGET_PROJECT_DIR
  else
...
project:
  ...
  source_repository:
    url: https://github.com/laplacian-arch/api-service.domain-model.git
    branch: master
  module_repositories:
    local:
      ../../../mvn-repo
...
show_usage () {
cat << 'END'
Usage: ./scripts/generate-api-service-domain-model.sh [OPTION]...
  -h, --help
