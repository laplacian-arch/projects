...
TARGET_PROJECT_DIR="$PROJECT_BASE_DIR/subprojects/java-stack-service-project-template"
...
sync_source_with_repository() {
  ...
  then
    ...
    git clone -b master \
        https://github.com/laplacian-arch/java-stack-service.project-template.git \
        $TARGET_PROJECT_DIR
  else
...
project:
  ...
  description:
    ...
    zh: |
      The java-stack-service.project-template project.
  source_repository:
    url: https://github.com/laplacian-arch/java-stack-service.project-template.git
    branch: master
  module_repositories:
    local:
      ../../../mvn-repo
...
show_usage () {
cat << 'END'
Usage: ./scripts/generate-java-stack-service-project-template.sh [OPTION]...
  -h, --help
