...
TARGET_PROJECT_DIR="$PROJECT_BASE_DIR/subprojects/api-service-springboot2-template"
...
sync_source_with_repository() {
  ...
  then
    ...
    git clone -b master \
        https://github.com/laplacian-arch/api-service.springboot2-template.git \
        $TARGET_PROJECT_DIR
  else
...
project:
  ...
  type: template
  namespace: laplacian_arch.api_service_springboot2_template
  version: '1.0.0'
  description:
    en: |
  ...
  source_repository:
    url: https://github.com/laplacian-arch/api-service.springboot2-template.git
    branch: master
  module_repositories:
    local:
      ../../../mvn-repo
...
show_usage () {
cat << 'END'
Usage: ./scripts/generate-api-service-springboot2-template.sh [OPTION]...
  -h, --help
