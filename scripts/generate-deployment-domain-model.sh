...
TARGET_PROJECT_DIR="$PROJECT_BASE_DIR/subprojects/deployment-domain-model"
...
sync_source_with_repository() {
  ...
  then
    ...
    git clone -b master \
        https://github.com/laplacian-arch/deployment.domain-model.git \
        $TARGET_PROJECT_DIR
  else
...
project:
  ...
  description:
    ...
    zh: |
      该模型表示系统的部署配置。
  source_repository:
    url: https://github.com/laplacian-arch/deployment.domain-model.git
    branch: master
  module_repositories:
    local:
      ../../../mvn-repo
  ...
  plugins:
  - group: laplacian-arch
    name: api-service.domain-model-plugin
    version: '1.0.0'
  models:
  - group: laplacian-arch
...
show_usage () {
cat << 'END'
Usage: ./scripts/generate-deployment-domain-model.sh [OPTION]...
  -h, --help
