...
TARGET_PROJECT_DIR="$PROJECT_BASE_DIR/subprojects/application-model-project-template"
...
sync_source_with_repository() {
  ...
  then
    ...
    git clone -b master \
        https://github.com/laplacian-arch/application-model.project-template.git \
        $TARGET_PROJECT_DIR
  else
...
project:
  ...
  description:
    ...
    zh: |
      应用功能模型开发项目的项目模板。
  source_repository:
    url: https://github.com/laplacian-arch/application-model.project-template.git
    branch: master
  module_repositories:
    local:
      ../../../mvn-repo
...
show_usage () {
cat << 'END'
Usage: ./scripts/generate-application-model-project-template.sh [OPTION]...
  -h, --help
